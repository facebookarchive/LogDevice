...
| rocksdb-partition-partial-compaction-file-num-threshold-old | don't consider file ranges for partial compactions (used during rebuilding) that are shorter than this for old partitions (>1d old). | 10 | server&nbsp;only |
...
| max-protocol | maximum version of LogDevice protocol that the server/client will accept | 93 |  |
