...
| sequencer-batching-compression | Compression setting for sequencer batching (if used). | zstd | requires&nbsp;restart, server&nbsp;only |
...
| rocksdb-min-manual-flush-interval | How often a background thread will flush buffered writes if either the data age, partition idle, or data amount triggers indicate a flush should occur. 0 disables all manual flushes | 120s | server&nbsp;only |
...
| store-timeout | timeout for attempts to store a record copy on a specific storage node. This value is used by sequencers only and is NOT the client request timeout. | 500ms..1min | server&nbsp;only |
