...
| metadata-log-trim-interval | How often periodic trimming of metadata logs should run. Zero value prevents it from running at all.  | 0ms | requires&nbsp;restart, server&nbsp;only |
...
| use-legacy-eventbase | Use libevent2 based event base to create EventLoop threadpool in logdevice. | false | requires&nbsp;restart |
