...
bool IsLogEmptyTest::trimRecords(std::vector<uint64_t> log_ids,
...
bool IsLogEmptyTest::getTailLSNs(std::vector<uint64_t> log_ids,
                                 std::vector<lsn_t>& tail_lsns_out) {
  Semaphore sem;
  std::atomic<bool> failed(false);
  ...
  for (IsLogEmptyResult& expected : expected_results) {
    int rv =
        static_cast<ClientImpl*>(client_.get())
            ->isLogEmpty(logid_t(expected.log_id), [&](Status st, bool empty) {
...
TEST_F(IsLogEmptyTest, NonExistingLogsStaticSeqPlacement) {
...
TEST_F(IsLogEmptyTest, NonExistingLogsDynamicSeqPlacement) {
  // Make timeout 10s so test doesn't time out.
  meta_api_timeout_sec = 20;
...
TEST_F(IsLogEmptyTest, SingleEpochTest) {
  init();
...
TEST_F(IsLogEmptyTest, FewEpochTest) {
  init();
...
TEST_F(IsLogEmptyTest, ManyEpochTest) {
  init();
