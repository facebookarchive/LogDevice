...
TEST_F(AdminAPILowLevelTest, LogTreeReplicationInfo) {
  ...
  std::map<std::string, int32_t> narrowest_expected{{"NODE", 2}};
  ...
  ASSERT_EQ("NODE", tolerable_failure_domains.get_domain());
...
TEST_F(AdminAPILowLevelTest, SettingsAPITest) {
  ...
  ASSERT_EQ("20min", rebuilding_window_setting.currentValue);
