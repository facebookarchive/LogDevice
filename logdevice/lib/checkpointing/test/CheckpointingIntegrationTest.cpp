...
TEST_F(CheckpointingIntegrationTest, SyncReaderTest) {
  cluster_->waitForRecovery();
  auto reader = client_->createReader(2);
  auto store =
      CheckpointStoreFactory().createFileBasedCheckpointStore(store_path_);
  ...
  store = CheckpointStoreFactory().createFileBasedCheckpointStore(store_path_);
  reader_ = CheckpointedReaderFactory().createSyncCheckpointedReader(
      "customer", std::move(reader), std::move(store), {});
...
TEST_F(CheckpointingIntegrationTest, AsyncReaderTest) {
  cluster_->waitForRecovery();
