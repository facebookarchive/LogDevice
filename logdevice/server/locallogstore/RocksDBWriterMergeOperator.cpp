...
void combine(const Slice& with_payload_slice,
  ...
  OffsetMap amend_offsets_wihtin_epoch;
  ...
  if (amend_flags & LocalLogStoreRecordFormat::FLAG_OFFSET_WITHIN_EPOCH) {
    ...
    if (amend_offsets_wihtin_epoch.isValid()) {
      offsets_within_epoch = std::move(amend_offsets_wihtin_epoch);
    }
  }
