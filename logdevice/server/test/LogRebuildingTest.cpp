...
// state machine.
TEST_F(LogRebuildingTest, RebuildingFilter) {
  init();
  ...
  logid_t log(42);
  LogRebuilding::RebuildingReadFilter filter(rebuilding_set, log);
  ...
  LogRebuilding::RebuildingReadFilter filter2(rebuilding_set, log);
...
// Verify the special copyset and record filter used by the LogRebuilding
// state machine.
TEST_F(LogRebuildingTest, RebuildingFilterDonorDirty) {
  init();
