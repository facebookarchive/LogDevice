    ...
    ticket.postCallbackRequest([st,
      ...
      switch (st) {
        case E::OK:
          ...
          s->processSnapshot(payload, snapshot_attrs);
          break;
        case E::EMPTY:
...
template <typename T, typename D>
void ReplicatedStateMachine<T, D>::activateSnapshotTimer() {
  if (get_snapshot_timer_ && !get_snapshot_timer_->isActive()) {
...
template <typename T, typename D>
void ReplicatedStateMachine<T, D>::initSnapshotTimer() {
  if (!get_snapshot_timer_) {
    rsm_info(rsm_type_, "Creating timer to fetch snapshots periodically ...");
...
template <typename T, typename D>
void ReplicatedStateMachine<T, D>::cancelSnapshotTimer() {
  if (get_snapshot_timer_) {
    get_snapshot_timer_->cancel();
  }
}
...
void ReplicatedStateMachine<T, D>::activateGracePeriodForSnapshotting() {
  ...
  bool snapshot_store_allows{true};
  if (snapshot_store_ && !snapshot_store_->isWritable()) {
    snapshot_store_allows = false;
  }
  ...
  if (snapshot_store_allows && !snapshotting_timer_.isActive()) {
    snapshotting_timer_.activate(snapshotting_duration);
  }
}
