...
namespace {
const std::string
makeConfigForLayoutTest(int range_start,
                        int range_end,
                        std::map<std::string, std::string> layouts) {
  const char* format = R"config(
{
  ...
  if (!layouts.empty()) {
    for (auto it : layouts) {
      if (!it.second.empty()) {
        props += folly::stringPrintf(
            "\"%s\": \"%s\",\n", it.first.c_str(), it.second.c_str());
      }
    }
  }
  return folly::stringPrintf(format, range_start, range_end, props.c_str());
}

void testLayout(int range_start,
                int range_end,
                const std::string& layout,
                const std::string& alternative_layout,
                const ConfigParserOptions& options,
                std::vector<int> expected_range_sizes) {
  std::map<std::string, std::string> layouts = {
      {"layout", layout}, {"alternative_layout", alternative_layout}};

  std::string configStr =
      makeConfigForLayoutTest(range_start, range_end, layouts);

  auto parsed = parser::parseJson(configStr);
  // Make sure the parsed string is actually an object
  ASSERT_TRUE(parsed.isObject());
  auto config = Configuration::fromJson(parsed, nullptr, nullptr, options);
  // check that the configuration could be parsed.
  ASSERT_NE(nullptr, config);
  ASSERT_NE(nullptr, config->serverConfig());
  ASSERT_NE(nullptr, config->logsConfig());
  ASSERT_NE(nullptr, config->localLogsConfig());
  const LogsConfig::LogGroupInDirectory* log;

  size_t num_ranges = expected_range_sizes.size();
  if (layout.empty()) {
    ASSERT_EQ(num_ranges, 1);
  }
  size_t max_range_size = (range_end - range_start + 1) / num_ranges;

  std::map<std::string, logid_range_t> cmp_ranges;
  for (int i = range_start; i <= range_end; ++i) {
    int slot = (i - range_start) / max_range_size;
    ld_info("log %d - slot %d - num_ranges %lu - max size %lu",
            i,
            slot,
            num_ranges,
            max_range_size);

    // check that the range index is valid
    ASSERT_TRUE(slot < num_ranges);
    if (expected_range_sizes[slot] == 0) {
      // make sure if the log is not expected, it does not exist in the
      // config object
      ASSERT_FALSE(config->logsConfig()->logExists(logid_t(i)));
      continue;
    }

    // decrement size to check we've seen all expected logs
    --expected_range_sizes[slot];

    // check that the log exists in the config object
    ASSERT_TRUE(config->logsConfig()->logExists(logid_t(i)));
    log = config->getLogGroupInDirectoryByIDRaw(logid_t(i));
    ASSERT_NE(nullptr, log);

    // check that the name of the range is the name of the group followed by
    // slash and the range index.
    std::string expected = "/layout_test";
    if (!layout.empty()) {
      expected += "/" + folly::to<std::string>(slot);
    }
    EXPECT_EQ(expected, log->getFullyQualifiedName());

    // Inserting in the range map. If an entry exists, bumping the upper bound
    // of the range
    auto insert_res = cmp_ranges.insert(std::make_pair(
        log->getFullyQualifiedName(), std::make_pair(logid_t(i), logid_t(i))));
    if (!insert_res.second) {
      insert_res.first->second.second = logid_t(i);
    }
  }

  if (layout.empty()) {
    ASSERT_EQ(cmp_ranges.size(), 1);
    ASSERT_EQ(cmp_ranges.begin()->second,
              config->logsConfig()->getLogRangeByName("layout_test"));
  } else {
    // check that getLogRangesByNamespace results contains all of the ranges
    auto ranges = config->logsConfig()->getLogRangesByNamespace("layout_test");
    ASSERT_EQ(cmp_ranges, ranges);
  }

  // check that the size of the ranges are now all 0, to make sure that we've
  // seen all of the logs we were expecting.
  auto it = std::max_element(
      expected_range_sizes.begin(), expected_range_sizes.end());
  ASSERT_TRUE(it != expected_range_sizes.end() && *it == 0);
}
...
TEST(ConfigurationTest, Layout) {
  using namespace facebook::logdevice;

  std::shared_ptr<Configuration> config(
      Configuration::fromJsonFile(TEST_CONFIG_FILE("layout.conf")));
  ...
  for (int i = 1; i <= 12; ++i) {
    ...
    ASSERT_TRUE(config->logsConfig()->logExists(logid_t(i)));
    ASSERT_NE(nullptr, log);
    EXPECT_EQ(1000, log->log_group->attrs().maxWritesInFlight().value());
  ...
  auto ranges = config->logsConfig()->getLogRangesByNamespace("a");
  ASSERT_EQ(cmp_ranges, ranges);
  ...
  for (int i = 13; i <= 24; ++i) {
    ...
    if ((i - 13) % 3 == 0) {
      ASSERT_NE(nullptr, log);
      ASSERT_TRUE(config->logsConfig()->logExists(logid_t(i)));
      EXPECT_EQ(2000, log->log_group->attrs().maxWritesInFlight().value());
      ...
      // Inserting in the range map. If an entry exists, bumping the upper bound
      // of the range
      auto insert_res = cmp_ranges.insert(
          std::make_pair(log->getFullyQualifiedName(),
                         std::make_pair(logid_t(i), logid_t(i))));
      if (!insert_res.second) {
        insert_res.first->second.second = logid_t(i);
      }
    } else {
      EXPECT_EQ(nullptr, log);
      EXPECT_FALSE(config->logsConfig()->logExists(logid_t(i)));
    }
  }

  std::map<std::string, logid_range_t> cmp_ranges2;
  for (int i = 8047; i <= 9046; ++i) {
    log = config->getLogGroupInDirectoryByIDRaw(logid_t(i));
    if ((i - 8047) % 20 < 7) {
      ASSERT_NE(nullptr, log);
      ASSERT_TRUE(config->logsConfig()->logExists(logid_t(i)));
      EXPECT_EQ(3000, log->log_group->attrs().maxWritesInFlight().value());
      ...
      // Inserting in the range map. If an entry exists, bumping the upper bound
      // of the range
      auto insert_res = cmp_ranges2.insert(
          std::make_pair(log->getFullyQualifiedName(),
                         std::make_pair(logid_t(i), logid_t(i))));
      if (!insert_res.second) {
        insert_res.first->second.second = logid_t(i);
  ...
  ranges = config->logsConfig()->getLogRangesByNamespace("c");
  ASSERT_EQ(cmp_ranges2, ranges);

  // fetching all logs in config
  ranges = config->logsConfig()->getLogRangesByNamespace("");
  cmp_ranges.insert(cmp_ranges2.begin(), cmp_ranges2.end());

  ASSERT_EQ(cmp_ranges.size(), ranges.size());
  ASSERT_EQ(cmp_ranges, ranges);
...
TEST(LayoutConfigurationTest, SingleRangeSize) {
  // test layout with all ranges of default size 1,
  // and one range of size 2

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 1:2";
  std::vector<int> expected_range_sizes({1, 2, 1, 1});

  testLayout(range_start,
             range_end,
             layout,
             "",
             ConfigParserOptions(),
...
TEST(LayoutConfigurationTest, AlternativeLayout) {
  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 1:2,2:4";
  std::string alternative_layout = "4x5 2:3";
  std::vector<int> expected_range_sizes({1, 1, 3, 1});
  ConfigParserOptions options;
  options.alternative_layout_property = "alternative_layout";
...
TEST(LayoutConfigurationTest, AlternativeLayoutLowerAppbuckets) {
  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 1:2,2:4";
  std::string alternative_layout = "3x5 1:2,2:4";
  std::vector<int> expected_range_sizes({1, 2, 4, 0});
  ConfigParserOptions options;
  options.alternative_layout_property = "alternative_layout";
...
TEST(LayoutConfigurationTest, AlternativeLayoutLowerAppbucketsUsingOverride) {
  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 1:2,2:4";
  std::string alternative_layout = "4x5 1:2,2:4,3:0";
  std::vector<int> expected_range_sizes({1, 2, 4, 0});
  ConfigParserOptions options;
  options.alternative_layout_property = "alternative_layout";
...
TEST(LayoutConfigurationTest, InvalidAlternativeLayout) {
  int range_start = 1;
  int range_end = 20;
  ConfigParserOptions options;
  options.alternative_layout_property = "alternative_layout";
  ...
  std::string layout = "4x5 1:2,2:4";
  std::string alternative_layout = "4x5 1:3,2:3";
  std::map<std::string, std::string> layouts = {
      {"layout", layout}, {"alternative_layout", alternative_layout}};
  ...
  std::unique_ptr<Configuration> config(
      Configuration::fromJson(configStr, nullptr, nullptr, options));

  ASSERT_EQ(nullptr, config->logsConfig());
  EXPECT_EQ(err, E::INVALID_CONFIG);

  // different maximum range size
  layouts["alternative_layout"] = "4x6 1:2,2:4";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);

  config = Configuration::fromJson(configStr, nullptr, nullptr, options);

  ASSERT_EQ(nullptr, config->logsConfig());
  EXPECT_EQ(err, E::INVALID_CONFIG);

  // high range count
  layouts["alternative_layout"] = "5x5 1:2,2:4";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);
  ...
  // range #3 not overridden in layout and larger than default
  layouts["alternative_layout"] = "4x5 1:2,2:4,3:3";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);
  ...
  // range #1 larger than in layout
  layouts["alternative_layout"] = "4x5 1:3,2:4";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);
  ...
  // range #0 larger than maximum size
  layouts["alternative_layout"] = "4x5 0:6,1:2,2:4";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);
  ...
  // range #5 beyond limit (only 4 ranges allowed here)
  layouts["alternative_layout"] = "4x5 1:2,2:4,5:3";
  configStr = makeConfigForLayoutTest(range_start, range_end, layouts);
...
TEST(LayoutConfigurationTest, AlternativeLayoutButNoLayout) {
  int range_start = 1;
  int range_end = 20;
  ConfigParserOptions options;
  options.alternative_layout_property = "alternative_layout";

  // valid layout - no override
  testLayout(range_start, range_end, "", "1x20", options, {1});

  // valid layout - overrides range size to 3
  testLayout(range_start, range_end, "", "1x20 0:3", options, {3});

  // valid layout - max size is ignored as long as it's greater than range size
  testLayout(range_start, range_end, "", "1x1024", options, {1});

  // Alternative layout defines more than 1 range
  std::map<std::string, std::string> layouts = {
      {"layout", ""}, {"alternative_layout", "4x5"}};
  ...
  // Alternative layout defines 1 range larger than actual log range
  layouts = {{"layout", ""}, {"alternative_layout", "1x20 0:21"}};
...
TEST(LayoutConfigurationTest, HigherDefaultSingleRangeSize) {
  // test layout with all ranges of overridden default size 3,
  // and one range of size 4

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5@3 1:4";
  std::vector<int> expected_range_sizes({3, 4, 3, 3});
...
TEST(LayoutConfigurationTest, HigherDefaultSingleLowerRangeSize) {
  // test layout with all ranges of overridden default size 3,
  // and one range of size 2

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5@3 1:2";
  std::vector<int> expected_range_sizes({3, 2, 3, 3});
...
TEST(LayoutConfigurationTest, ZeroRangeSize) {
  // test layout with all ranges of default size 1,
  // and one empty range (size 0)

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 2:0";
  std::vector<int> expected_range_sizes({1, 1, 0, 1});
...
TEST(LayoutConfigurationTest, MultipleRangeSize) {
  // test layout with multiple ranges specified

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 0:3,2:2,3:4";
  std::vector<int> expected_range_sizes({3, 1, 2, 4});
...
TEST(LayoutConfigurationTest, MissingSize) {
  // test missing size in specifier

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 2";

  std::string configStr =
      makeConfigForLayoutTest(range_start, range_end, {{"layout", layout}});

  std::shared_ptr<Configuration> config(Configuration::fromJson(
      configStr, nullptr, nullptr, ConfigParserOptions()));

  ASSERT_EQ(nullptr, config->logsConfig());
  EXPECT_EQ(err, E::INVALID_CONFIG);
}

TEST(LayoutConfigurationTest, SizeTooBig) {
  // test size greater than max range size

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 2:6";
...
TEST(LayoutConfigurationTest, SizeNotANumber) {
  // test invalid size (not a number)

  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 2:bla";
...
TEST(LayoutConfigurationTest, IndexNotANumber) {
  int range_start = 1;
  int range_end = 20;
  std::string layout = "4x5 bla:2";
...
TEST(ConfigurationTest, BadLayout) {
  using namespace facebook::logdevice;

  std::shared_ptr<Configuration> config(
      Configuration::fromJsonFile(TEST_CONFIG_FILE("bad_layout.conf")));
