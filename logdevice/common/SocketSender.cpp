...
class DisconnectedClientCallback : public SocketCallback {
 public:
  explicit DisconnectedClientCallback(SocketSender* sender) : sender_(sender) {}
...
class SenderImpl {
 public:
  ...
  void ensureCleanupTimerActive(SocketSender& sender) {
...
void SocketSender::onCompletedMessagesAvailable(void* arg, short) {
  SocketSender* self = reinterpret_cast<SocketSender*>(arg);
  self->deliverCompletedMessages();
}

int SocketSender::addClient(int fd,
  ...
  if (shutting_down_) {
    ...
    ld_error("SocketSender is shut down");
    err = E::SHUTDOWN;
    return -1;
  }
...
void SocketSender::noteBytesQueued(size_t nbytes,
                                   PeerType peer_type,
...
void SocketSender::noteBytesDrained(size_t nbytes,
                                    PeerType peer_type,
                                    folly::Optional<MessageType> message_type) {
...
ssize_t SocketSender::getTcpSendBufSizeForClient(ClientID client_id) const {
...
ssize_t
SocketSender::getTcpSendBufOccupancyForClient(ClientID client_id) const {
  auto it = impl_->client_conns_.find(client_id);
  if (it == impl_->client_conns_.end()) {
    return -1;
  }
...
void SocketSender::eraseDisconnectedClients() {
...
bool SocketSender::canSendToImpl(const Address& addr,
                                 TrafficClass tc,
...
int SocketSender::sendMessageImpl(std::unique_ptr<Message>&& msg,
                                  const Address& addr,
                                  BWAvailableCallback* on_bw_avail,
  ...
  if (rv != 0) {
    ...
    if (!no_warning) {
      RATELIMIT_LEVEL(
          ...
          SocketSender::describeConnection(addr).c_str(),
          error_name(err));
    }
  }
  return rv;
...
int SocketSender::sendMessageImpl(std::unique_ptr<Message>&& msg,
  ...
  if (!isHandshakeMessage(msg->type_) &&
    ...
    RATELIMIT_WARNING(std::chrono::seconds(1),
                      10,
                      "ENOBUFS for SocketSender. Peer type: %s. "
                      "Current SocketSender outbuf usage: %zu. "
                      "Current SocketSender peer outbuf usage : %zu",
...
Connection* SocketSender::findServerConnection(node_index_t idx) const {
  ld_check(idx >= 0);
...
SocketSender::findClientConnection(const ClientID& client_id) const {
  auto it = impl_->client_conns_.find(client_id);
  if (it == impl_->client_conns_.end()) {
    return nullptr;
  }
...
void SocketSender::resetServerSocketConnectThrottle(node_index_t node_id) {
...
void SocketSender::setPeerShuttingDown(node_index_t node_id) {
  auto conn = findServerConnection(node_id);
  if (conn != nullptr) {
...
int SocketSender::registerOnConnectionClosed(const Address& addr,
                                             SocketCallback& cb) {
  Connection* conn;
...
void SocketSender::flushOutputAndClose(Status reason) {
...
int SocketSender::closeConnection(const Address& addr, Status reason) {
  if (addr.isClientAddress()) {
...
int SocketSender::closeClientSocket(ClientID cid, Status reason) {
  ld_check(cid.valid());
...
int SocketSender::closeServerSocket(node_index_t peer, Status reason) {
...
void SocketSender::closeAllSockets() {
...
void SocketSender::beginShutdown() {
  shutting_down_ = true;
  flushOutputAndClose(E::SHUTDOWN);
}

void SocketSender::forceShutdown() {
  if (!shutting_down_) {
    ld_warning("Force shutdown of SocketSender w/o graceful shutdown attempt");
    shutting_down_ = true;
  }
  closeAllSockets();
...
bool SocketSender::isShutdownCompleted() const {
...
bool SocketSender::isClosed(const Address& addr) const {
  if (addr.isClientAddress()) {
    auto pos = impl_->client_conns_.find(addr.id_.client_);
...
int SocketSender::checkServerConnection(node_index_t node) const {
...
int SocketSender::checkClientConnection(ClientID cid,
...
int SocketSender::connect(node_index_t nid) {
  if (shutting_down_) {
    err = E::SHUTDOWN;
    return -1;
  }
...
bool SocketSender::useSSLWith(node_index_t node_id,
...
Connection* SocketSender::initServerConnection(NodeID nid,
...
Connection* FOLLY_NULLABLE SocketSender::getConnection(const ClientID& cid) {
  if (shutting_down_) {
    err = E::SHUTDOWN;
    return nullptr;
  }
...
Connection* FOLLY_NULLABLE SocketSender::getConnection(const NodeID& nid,
                                                       const Message& msg) {
  if (shutting_down_) {
    err = E::SHUTDOWN;
    return nullptr;
...
Connection* FOLLY_NULLABLE SocketSender::getConnection(const Address& addr,
...
Connection* FOLLY_NULLABLE SocketSender::findConnection(const Address& addr) {
  if (addr.isClientAddress()) {
...
SocketSender::getConnectionInfo(const Address& address) const {
...
SocketSender::extractPeerIdentity(const Address& addr) {
...
int SocketSender::noteDisconnectedClient(ClientID client_name) {
...
void DisconnectedClientCallback::operator()(Status st, const Address& name) {
  ld_debug(
      "SocketSender's DisconnectedClientCallback called for Connection  %s "
      "with status %s",
...
void SocketSender::noteConfigurationChanged(
...
void SocketSender::onSettingsUpdated(
    std::shared_ptr<const Settings> new_settings) {
  settings_.swap(new_settings);
  disconnectFromNodesThatChangedAddressOrGeneration();
}

void SocketSender::disconnectFromNodesThatChangedAddressOrGeneration() {
...
bool SocketSender::bytesPendingLimitReached(const PeerType peer_type) const {
...
void SocketSender::queueMessageCompletion(std::unique_ptr<Message> msg,
                                          const Address& to,
                                          Status s,
...
std::string SocketSender::dumpQueuedMessages(Address addr) const {
...
void SocketSender::forAllConnections(
...
void SocketSender::cleanupConnections() {
  ...
  if (sockets_closed) {
    RATELIMIT_WARNING(
        std::chrono::seconds(10),
        10,
        "SocketSender closed %lu connections of which %lu had slow network"
...
void SocketSender::fillDebugInfo(InfoSocketsTable& table) const {
  forAllConnections(
